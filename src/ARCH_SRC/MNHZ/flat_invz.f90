...
!JUAN Z_SPLITTING
  ...
  IF (  IAND(NZ_SPLITTING,2) > 0 ) THEN 
     ...
     CALL SECOND_MNH2(T1)
     T_MAP_B_SX_YP2_ZP1  = T_MAP_B_SX_YP2_ZP1 + T1 - T0
  END IF ! NZ_SPLITTING
  !
  !JUAN Z_SPLITTING
  !
  ...
  IF (  IAND(NZ_SPLITTING,2) > 0 ) THEN 
     ...
     CALL SECOND_MNH2(T1)
     T_MAP_SX_YP2_ZP1_SXP2_Y_ZP1  = T_MAP_SX_YP2_ZP1_SXP2_Y_ZP1 + T1 - T0
  END IF ! NZ_SPLITTING
  !
  !JUAN Z_SPLITTING
  !
  ...
  IF (( IAND(NZ_SPLITTING,2) > 0 ) .AND. ( IAND(NZ_SPLITTING,16) > 0 ) ) THEN
     ...
     CALL SECOND_MNH2(T1)
     T_MAP_SXP2_Y_ZP1_B  = T_MAP_SXP2_Y_ZP1_B + T1 - T0
     !    
     !       singular matrix case : the last term is computed by setting the 
     !       average of the pressure field equal to zero.
...
!
  IF  ( ( IAND(NZ_SPLITTING,2) > 0 ) .AND. (  IAND(NZ_SPLITTING,8) > 0 )) THEN
     ...
     CALL SECOND_MNH2(T1)
     T_MAP_SXP2_Y_ZP1_SXP2_YP1_Z  = T_MAP_SXP2_Y_ZP1_SXP2_YP1_Z + T1 - T0
     !    
     !       singular matrix case : the last term is computed by setting the 
     !       average of the pressure field equal to zero.
...
!
  ...
  IF  ( ( IAND(NZ_SPLITTING,2) > 0 ) .AND.(  IAND(NZ_SPLITTING,16) > 0 ) ) THEN
     ...
     CALL SECOND_MNH2(T1)
     T_MAP_B_SXP2_Y_ZP1  = T_MAP_B_SXP2_Y_ZP1 + T1 - T0
  ENDIF
! JUAN P1/P2 SPLITTING
  IF  ( ( IAND(NZ_SPLITTING,2) > 0 ) .AND. (  IAND(NZ_SPLITTING,8) > 0 ) ) THEN
     !
...
!TEST     CALL REMAP_SXP2_YP1_Z_SXP2_Y_ZP1_ll(ZBAND_SXP2_YP1_ZR,ZBAND_SXP2_Y_ZP1RBIS,IINFO_ll)
     CALL SECOND_MNH2(T1)
     T_MAP_SXP2_YP1_Z_SXP2_Y_ZP1  = T_MAP_SXP2_YP1_Z_SXP2_Y_ZP1 + T1 - T0
  ENDIF
  IF (  IAND(NZ_SPLITTING,2) > 0 )  THEN
     ...
     CALL SECOND_MNH2(T1)
     T_MAP_SXP2_Y_ZP1_SX_YP2_ZP1  = T_MAP_SXP2_Y_ZP1_SX_YP2_ZP1 + T1 - T0
     !
     IF (HLBCX(1) == 'CYCL') THEN
        CALL FFT991(ZBAND_SX_YP2_ZP1(1,1,1),ZWORK_SX_YP2_ZP1,PTRIGSX,KIFAXX,INC1_SX_YP2_ZP1,INC2_SX_YP2_ZP1,  &
             IIMAX,ILOT_SX_YP2_ZP1,+1 )
  ...
  IF (  IAND(NZ_SPLITTING,2) > 0 ) THEN
     ...
     CALL SECOND_MNH2(T1)
     T_MAP_SX_YP2_ZP1_B  = T_MAP_SX_YP2_ZP1_B + T1 - T0
     PF_1_Y = ZBAND_B
  END IF
  !
  ...
  !JUAN Z_SPLITTING
  NULLIFY(TZFIELDS_ll)
  CALL ADD3DFIELD_ll(TZFIELDS_ll,PF_1_Y )
