...
USE MODE_SPLITTINGZ_ll , ONLY : GET_DIM_EXTZ_ll,GET_ORZ_ll
!JUAN
!
!
IMPLICIT NONE
...
IF ( CEQNSYS == 'DUR' .OR. CEQNSYS == 'MAE' ) THEN
  DO JJ = IJB,IJE
    DO JI = IIB,IIE
     PRHOM(:) = PRHOM(:) + PRHODJ(JI,JJ,:)*XCPD*PTHVREF(JI,JJ,:)*ZINVMEAN       
    END DO
  END DO
ELSEIF ( CEQNSYS == 'LHE' ) THEN
  DO JJ = IJB,IJE
    DO JI = IIB,IIE
     PRHOM(:) = PRHOM(:) + PRHODJ(JI,JJ,:)*ZINVMEAN          
    END DO
  END DO
END IF
!
!  global sum
CALL REDUCESUM_ll(PRHOM,IINFO_ll)
!
!------------------------------------------------------------------------------
!
...
DO JK = IKB-1,IKE
  DO JJ = IJB,IJE
    DO JI = IIB,IIE
      ZDZM(JK) = ZDZM(JK)+(PZZ(JI,JJ,JK+1)-PZZ(JI,JJ,JK))*ZINVMEAN
    END DO
  END DO
END DO
ZDZM(IKE+1) = ZDZM(IKE)
!
!  global sum
CALL REDUCESUM_ll(ZDZM,IINFO_ll)
!
!
...
!JUAN in _SXP2_YP1_Zsplitting slides so need PBF_SXP2_YP1_Z 
  DO JK= IKB,IKE
!!$    DO JJ= 1, IYMODE_SXP2_YP1_Z_ll
!!$      DO JI= 1, IXMODE_SXP2_YP1_Z_ll
!!$    DO JJ= IJB,IJE
!!$      DO JI= IIB, IIE
...
!JUAN Z_SPLITTING
! second coefficent is meaningless in cyclic case
IF (HLBCX(1) == 'CYCL' .AND. L2D) PBFB(2,:,:)=1.
IF (HLBCX(1) == 'CYCL' .AND. .NOT.(L2D) .AND. LWEST_ll(HSPLITTING='B')) PBFB(:,2,:)=1.
IF (HLBCY(1) == 'CYCL' .AND. .NOT.(L2D)) PBFB(2,:,:)=1.
!JUAN Z_SPLITTING
!
!------------------------------------------------------------------------------
